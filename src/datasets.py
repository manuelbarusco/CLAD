...
ONLY_CLASS_LABELS_BMAD = ["Chest_AD", "Histopathology_AD", "Retina_OCT2017_AD"]
...
def load_dataset(parameters,type_dataset,normalize=True):
    ...
    if type_dataset=="mvtec":
        ...
        opt.apply_rotation=False
        dataset_test = BMADDataset(opt,is_train=False)#entire test dataset
    else:
...
def natural_sort_key(s):
    ...
    ad es.: 
        ...
        ordinamento con sorted (con natural_sort_key): ['img1.png', 'img2.png', 'img10.png', 'IMG11.png', 'img20.png']
...
class BMADDataset(Dataset):
    """
    Classe che gestisce il dataset BMAD.
    """
    ...
    def load_dataset_folders(self):
        ...
        for class_name in BMAD_CLASS_NAMES:
            ...
            anomaly_info = [anomaly_info[i] for i in indices]
    ...
    def load_dataset_folder(self):
        ...
        index = np.where( np.asarray(BMAD_CLASS_NAMES)==self.class_name)[0][0]
        index_class = index
            ...
            if img_type == 'good':
                ...
                if self.only_normal is False:
                    ...
                    if len(os.listdir(gt_dir)) != 0:
                        ...
                        gt_fpath_list = [os.path.join(gt_dir, img_fname[0] + '_mask' + img_fname[1]).replace('\\','/')
                                        for img_fname in img_fname_list]
                        mask.extend(gt_fpath_list)
                        y.extend([index_class] * len(img_fpath_list))
                        x.extend(img_fpath_list)
    ...
    def create_transform(self, opt, crp_size):
        ...
        """
        transform = [
            transforms.Resize((crp_size, crp_size)),
            transforms.ToTensor(),
